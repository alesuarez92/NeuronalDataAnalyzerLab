    ...
    methods
        ...
        function buildUI(app)
            ...
            [app.EphysPanel, chain] = app.workflowCard(contentGrid, ...
                ...
                'TDT recordings: extract LFP and MUA, then event-related potentials, CSD, spike sorting and rates.', ...
                'In: TDT tank / block folder  ·  Out: LFP .mat and MUA .mat', ...
                'Ephys Extract');
            app.ExtractEphysBtn = chainButton(chain, 1, '1   Extract (TDT)', @()ExtractEphysApp(), ...
                ['Input: TDT tank/block folder with Whis (stimulus) and xRAW streams; needs the TDT ' ...
                 'MATLAB SDK (README: Install the TDT SDK). Output: LFP .mat (lfp_data, lfp_channels, ' ...
            ...
            chainNote(chain, 'Brightness · Movement · ΔF/F · Flow speed · Kymograph · Vessel diameter');

            % --- Response features ---
            [app.CharPanel, chain] = app.workflowCard(contentGrid, ...
                'Response features  ·  Signal Characterization', ...
                'Measure each response: peak latency, onset delay, FWHM, AUC, rise and decay time.', ...

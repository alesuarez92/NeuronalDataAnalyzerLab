...
%
% Extra demos generated by core/demo/* (also reachable via DemoData.file):
...
classdef DemoData
    ...
    methods(Static)
        ...
        %       'groups' | 'intan' | 'openephys' | 'nwb'. Files live in
        ...
        %% extraFile - Demos kept as folders or several files (core/demo)
        function p = extraFile(kind)
            DemoData.ensureDemoPath();
            if strcmp(kind, 'groups')
        ...
        %% ensureDemoPath - Put core/demo (extra demo generators) on the path
        ...
        function files = writeAll(folder)
            ...
            save(files.lfpOscillations, '-struct', 's');

            s = demoImagingAdvanced();
            files.imagingAdvanced = fullfile(folder, 'demo_imaging_advanced.mat');
            save(files.imagingAdvanced, '-struct', 's');

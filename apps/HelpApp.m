    ...
    methods(Static)
        ...
        function s = textROIAnalysis()
            s = {
                ...
                '   RGB frames are converted to grayscale (mean of the colour channels).'
                '   A roiMask (logical H×W) in the .mat is used when no ROI is drawn.'
                ''
                '2. Preprocessing (optional)'
                '   • B&W 256: convert to 256-level grayscale (for fluorescence intensity).'
                '   • Smooth: Gaussian smooth each frame.'
                '   • Norm: normalize each frame to 0–1 (per-frame min/max).'
                ''
        ...
        function s = textSignalCharacterization()
            s = {
                ...
                '   ERP/average (t or t_lfp with lfp_data, or t, y), or generic time series (t, y).'
                ''
                '2. Set Parameters'
                '   Stimulus onset t0 (s): time of stimulus in the trace.'
                '   Baseline window: e.g. 0 0.05 for 0 to 0.05 s (reference level for all features;'
                '   falls back to the pre-onset mean if the window is empty).'
                '   Direction: Positive (peaks), Negative (troughs), or Auto (larger post-onset'
        ...
        function s = textMUAAnalysis()
            s = {
                ...
                'negative, or both. Methods: Standard (mean + k·SD), MAD'
                '(median + k·MAD), NEO (nonlinear energy operator; either'
                'polarity), Rolling MAD, or Percentile (99.9th). A short dead'
                'time (~0.3 ms) avoids double-counting; the refractory period'
                '(ms) is used for the per-cluster ISI quality check.'
                ''
                'Sorting: For each detected spike, a waveform snippet is extracted.'
                'Features are computed (PCA, ICA*, waveform, wavelet*, or t-SNE;'
                '*shown only when FastICA / Wavelet Toolbox is available).'
                'Clustering assigns spikes to units: K-means and GMM try 2–10'
                'clusters and keep the number with the best mean silhouette;'
                'DBSCAN is density-based (epsilon auto-tuned if left empty;'
                'unclustered spikes are labelled 0 = noise).'
                'Result: spike times and cluster ID per spike.'
                ''};
        end
    end
end

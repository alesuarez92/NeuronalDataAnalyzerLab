...
classdef ROIAnalysisApp < handle
    ...
    methods
        ...
        function buildUI(app)
            ...
            uilabel(app.HeaderPanel, 'Text', 'ROI / Coregistered Image Analysis', ...
                'Position', [T.headerPaddingH 18 450 24], ...
                'FontSize', T.fontTitle, 'FontWeight', 'bold', 'FontColor', T.headerTitleColor);
            uilabel(app.HeaderPanel, 'Text', 'ROI/line: brightness, movement, speed, ΔF/F, kymograph, vessel diameter.', ...
                'Position', [T.headerPaddingH 4 520 16], ...
                'FontSize', T.fontSubtitle, 'FontColor', T.headerSubtitleColor);
            uibutton(app.HeaderPanel, 'push', 'Text', 'Help', ...
                'Position', [920 12 72 32], 'BackgroundColor', [1 1 1], 'FontColor', T.headerBg, 'FontWeight', 'bold', ...
                'ButtonPushedFcn', @(~,~)HelpApp('ROI Analysis'));

            % --- Fixed controls: Load, preprocess, Draw ROI/Line, Compute ---
            row1 = uigridlayout(main, [1, 12], 'ColumnWidth', {88, '1x', 72, 72, 72, 82, 82, 160, 90}, 'ColumnSpacing', 8);
            ...
            imgGrid = uigridlayout(panelImg, [1, 1], 'Padding', [T.axesPanelPadding(1) T.axesPanelPadding(2) T.axesPanelPadding(3) T.axesPanelPadding(4)]);
            app.AxesImage = uiaxes(imgGrid);

            % --- Plot axes: padding for titles ---
            panelPlot = uipanel(main, 'Title', 'Result: time series or kymograph', ...
                'BackgroundColor', T.cardBg, 'BorderType', 'line', 'HighlightColor', T.cardBorder);
            plotGrid = uigridlayout(panelPlot, [1, 1], 'Padding', [T.axesPanelPadding(1) T.axesPanelPadding(2) T.axesPanelPadding(3) T.axesPanelPadding(4)]);
